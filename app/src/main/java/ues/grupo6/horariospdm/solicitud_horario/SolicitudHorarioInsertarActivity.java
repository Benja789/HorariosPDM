...
public class SolicitudHorarioInsertarActivity extends AppCompatActivity {
    ControlBD helper;
    ...
    public void insertarSolicitudHorario(View v) {
        String nombreGrupo = spinnerGrupo.getSelectedItem().toString();
        String nombreSalon = spinnerSalon.getSelectedItem().toString();
        String nombreCicloAcademico = spinnerCicloAcademico.getSelectedItem().toString();

        // Buscar el objeto TipoEvento correspondiente al nombre seleccionado
        Grupo grupoSeleccionado = buscarGrupoPorNombre(Integer.parseInt(nombreGrupo));
        Salon salonSeleccionado = buscarSalonPorNombre(nombreSalon);
        CicloAcademico cicloSeleccionado = buscarCicloAcademicoPorNombre(Integer.parseInt(nombreCicloAcademico));
        ...
        if (grupoSeleccionado != null && salonSeleccionado != null && cicloSeleccionado != null ) {
            ...
            solicitudHorario.setId_grupo(id_grupo);
            solicitudHorario.setId_salon(id_salon);
            solicitudHorario.setId_ciclo_academico(id_ciclo);
            solicitudHorario.setEstado_solicitud_horario(1);

            helper.abrir();
            String regInsertados = helper.insertarSolicitudHorario(solicitudHorario);
            helper.cerrar();
